...
subtitle: "Documentation du lot 1 de l'investissement (v2)"
...
knitr::include_graphics("images/maup.png")
```
...
knitr::include_graphics("images/contiguite.png")
```
...
### Les pôles des 53 plus grandes AAV
...
Le tableau de bord est une application Shiny, hébergée sur le Cluster Kube (plateforme interne Insee).
Depuis votre poste de travail, il suffit de suivre l'URL : http://au33dashboard.kube.developpement.insee.fr/
...
knitr::include_graphics( "images/TdB_vueEnsemble.png")
```
...
knitr::include_graphics( "images/casEtude4.png")
```

```{r casEtude5, fig.align = 'center',out.width="75%", fig.cap = "Fig8.",echo=FALSE}
knitr::include_graphics( "images/casEtude5.png")
```
...
* Forte baisse du poids démographique des QP dans la population totale de Strasbourg (-13 points) alors que les QP contribuent, par définition, très fortement au niveau de ségrégation total de la ville.
...
knitr::include_graphics( "images/casEtude6.png")
```

```{r casEtude7, fig.align = 'center',out.width="75%", fig.cap = "Fig10.",echo=FALSE}
knitr::include_graphics( "images/casEtude7.png")
```
...
knitr::include_graphics( "images/casEtude8.png")

```

### Quelques points de vigilance

* Faire attention lors de l'évocation du nombre d'habitants dans les QPV (portée politique importante). Les chiffres officiels sont publiés par l'Insee, et sont actuellement basés sur le RP2018. Les estimations disponibles *via* AU33 sont des approximations pour plusieurs raisons : 
...
* Pour la diffusion de chiffres, les règles d'arrondis de la source Filosofi font référence ("X:\HAB-FILOSOFI-DONNEES-NBL\Filosofi 2019\Table Ménages\documentation\Note_utilisateurs_2019_202203.pdf")
...
knitr::include_graphics( "images/entropie3D.png")
```
...
L'indice de ségrégation de Theil se définit comme la moyenne pondérée (par l’effectif du carreau) de l'écart  entre l'entropie de la ville et celle de chaque carreau la composant :
...
knitr::include_graphics( "images/exPedagoEntropie1.png")
```
...
knitr::include_graphics( "images/exPedagoEntropie2.png")
```
...
knitr::include_graphics( "images/exPedagoEntropie5.png")
```
...
* Filosofi 2012 : X:/HAB-PSAR-AU-AU33-DEV/au33_v2/data/input/passage_id_filo/12/N_infra/n_revdisp12.sas7bdat
* Filosofi 2013 : X:/HAB-PSAR-AU-AU33-DEV/au33_v2/data/input/passage_id_filo/13/n_infra/n_revdisp13.sas7bdat
...
### Table pour faire le lien id-Dirnoseq

Uniquement nécessaire pour les bases Filosofi 2014 - Filosofi 2019 : 

* X:/HAB-FILOSOFI-PROD/bases14/r_menages/ident_menages14.sas7bdat
* X:/HAB-FILOSOFI-PROD/bases15/r_menages/ident_menages15.sas7bdat
* X:/HAB-FILOSOFI-PROD/bases16/r_menages/ident_menages16.sas7bdat
* X:/HAB-FILOSOFI-PROD/bases17/r_menages/ident_menages17.sas7bdat
* X:/HAB-FILOSOFI-PROD/bases18/r_menages/ident_menages18.sas7bdat
* X:/HAB-FILOSOFI-PROD/bases19/r_menages/ident_menages19.sas7bdat


### Base du zonage en aires d'attraction des villes

https://www.insee.fr/fr/statistiques/fichier/4803954/AAV2020_au_01-01-2022.zip

### Base des contours géographiques des QPV

Base téléchargée à partir de Créacartes : 
"\\pd_as_ge_d1_50\ge_data_pd\creacartes_pd\fichiers-ihm\2021\franceentiere\qp_franceentiere_2021.shp"
...
* Les bases ménages étant nombreuses et volumineuses, on privilégie : 
    * le package `data.table`
    * La parallélisation des traitements les plus longs.

### Importation des bases ménages RFL et Filosofi

Traitement des bases ménages au format .sas7bdat sous SAS (script : **export_csv.sas**) :

* Sélection des seules variables d'intérêt (limiter la taille)
...
* L'identifiant du ménage (pour apparier des coordonnées géographiques fiables *a posteriori*, car celles déjà présentes dans les bases sont très souvent non renseignées).
  * De RFL 2004 à Filosofi 2012, on dispose directement du Dirnoseq (identifiant du local, intertemporel)
  * A partir de 2013, l'identifiant du ménage n'est plus le dirnoseq (pour des raisons de confidentialité des données). Il faudra alors rappatrier les dirnoseq a postériori, à l'aide de tables de passage.
...
### Restriction du champ a priori (DOM)

* On restreint l'analyse à la France métropolitaine + Martinique + La Réunion.

### Rappatriement des dirnoseq de 2014 à 2019

* Dezippage des dossiers compressés contenant les bases .sas7bdat à l'intérieur du coffre
* Importation sous R
* Jointure sur les bases ménages


### Conversion des COG en géographie 2020

Les pôles sont définis à partir du code géographique officiel (COG) des communes qui les composent. Dans les bases ménages RFL-Filosofi, ces COG sont renseignés en géographie courante. 

Afin d'éviter les problèmes liés aux fusions/séparations des communes, tous les COG sont convertis dans la géographie la plus récente (**2022** pour la dernière mise à jour). De plus, tous les COG spécifiques pour les arrondissements communaux de Paris-Lyon-Marseille sont remplacés par les COG de la ville entière afin d'éviter les incompatibilités pour les appariements à venir.

En pratique, il n'est pas toujours possible de gérer les séparation de commune (par exemple, un ménage de la base 2016 vivant dans une ancienne commune ayant laissé place à 2 communes distinctes...). Ces cas ne sont pas gérés. Néanmoins, nous nous sommes assurés que ce cas n'arrive jamais dans les 53 pôles étudiés depuis 2004 (à l'exception d'une commune du pôle de Rouen, mais ce cas ne pose pas de difficultés car les communes issues de cette séparation appartiennent aussiì au pôle).
...
Ces contrôles ont permis de soulever les problèmes ci-dessous. Le détail du résultat de ces contrôles est disponible dans le fichier suivant, dans le coffre de l’investissement sous AUS : **«\ X:/HAB-PSAR-AU-AU33-DEV/au33_v2/data/output/qualite_inputs/controle_qualite.html\ »**. À noter que les traitements sont réalisés en amont, dans un script R classique, car les performances sont très mauvaises (voire impossibles) en cas d'inclusion de ces traitements dans le script Rmarkdown.

### Correction des XY

Les coordonnées géographiques de RFL ne sont pas exploitables car faiblement renseignées. Ainsi, il convient d'apparier des coordonnées géographiques externes grâce au code Dirnoseq de chaque ménage (toujours bien renseigné).

En plus de trouver des XY pour les ménages des bases RFL, on souhaite obtenir, pour chaque ménage, sa géolocalisation la plus récente retenue en interne à l'Insee. De manière sous-jacente, on fait l'hypothèse que les campagnes de géolocalisation récentes sont de meilleure qualité car elle bénéficient de l'amélioration continue de techniques utilisées. Ainsi, même si un ménage posède déjà un couple XY dans nos bases, nous allons tenter de le remplacer par une géolocalisation plus récente dans la mesure du possible.

Dans un premier temps, on utilise une table de passage produite par la division logement (MOA de Fideli). Malheureusement, cette base ne porte que sur les locaux du champ Fideli, à savoir sur les année 2016-2019. Ainsi, beaucoup de ménages de nos bases plus anciennes ne sont pas répertoriés dans cette table de passage.

Pour ce faire, on utilise en complément une table de passage interne à la division DSAU. 

Cette base doit être légèrement retravaillée : 

* Elle présente des doublons "exacts" sur le couple dirnoseq*xy
* Elle présente des doublons "inexactes" : 1 dirnoseq associé à plusieurs couples xy

Pour finir, on retient en priorité les coordonnées xy issues de la table de la division logement (pour les ménages présents dans les deux bases).

Un faible nombre de ménages n'a pas de coordonnées géographiques malgré cet appariement. 
Ceci est essentiellement notable en 2004 : 35 000 ménages n'ont pas de coordonnées géographiques, ce qui ne représente cependant que 0,1 % de l'ensemble des ménages. Pour les autres millésimes, leur nombre est négligeable. Ces ménages n'ayant pas de coordonnées géographiques sont retirés des bases de données. Les conséquences sur les indicateurs et sur les cartes sont *a priori* infimes, voire inexistantes.
...
Toutes les bases corrigées sont stockées dans le répertoire : **«\ X:/HAB-PSAR-AU-AU33-DEV/au33_v2/data/output/lmen_cor.RDS\ »**.
...
Là encore, pour des raisons computationnelles, les résultats sont générés et enregistrées dans un flux R classique, et ils sont réimportés et affichés dans un script Rmarkdown dans un second temps.
...
knitr::include_graphics( "images/cylindrage.png")
```
...
Les résultats de cette opération sont disponibles dans le fichier suivant : **«\ X:/HAB-PSAR-AU-AU33-DEV/au33_v2/data/output/cylindrage/cylindrage.html\ »**



### Carroyage et algorithme de création des bases de résultats

#### Définition des pôles d'AAV à partir des COG 2022

Le pôle d'appartenance d'un ménage est repéré via son COG 2022 (issu des bases RFL-Filosofi et de nos corrections). Cette appartenance ne provient donc pas d'un traitement géographique.

#### Ajout d'informations sur le carreau d'appartenance d echaque ménage

Pour chaque année, chaque pôle et chaque taille de carreau : 

* On associe le ménage à son centroïde de carreau 
* On associe le ménage à son identifiant de carreau en norme Inspire
* On calcule le nombre de ménages du carreau auquel appartient chaque ménage
* On crée une indicatrice de filtrage (pour savoir si le carreau comporte au moins 20 ménages)

Cette opération est réalisée grâce à la fonction **f_inspire_filtre**.

Remarque : on conserve, à ce stade, des bases dont l'unité statistique est le ménage. 
...
* On récupère le contour des QP sur la surface du pôle
* On assimile les carreaux à leur centroïde
* On réalise l'intersection entre les centroïdes et le contour des QP (avec un buffer d'un demi-carreau)

Ainsi, on connait les carreaux intersectant les QP pour un pôle donné. Pour savoir si ces carreaux doivent être retenus comme des carreaux constitutifs des QP : 
...
* Pour les carreaux à cheval, ceux traversés par une frontière réelle d'un QP, on considère que le carreau est constitutif du QP si et seulement si les individus du carreau sont majoritairement géolocalisés à l'intérieur de la frontière réelle du QP. 


```{r carroyageQP, fig.align = 'center',out.width="100%", fig.cap = "Fig17. Schéma d'approximation des QPV dans un carroyage", echo=FALSE}
knitr::include_graphics( "images/shema_carreauxQP.jpg")
```
...
knitr::include_graphics( "images/carroyageQP_villejean.png")
```
...
A partir des bases ménages précédentes, les principaux résultats de l'investissement sont produit à l'aide de la fonction **f_carroyage** : 

- Des bases de carreaux les indicateurs suivant :
    - un identifiant Inspire
...
- Le revenu médian de chaque groupe de carreaux.
...
La fonction **fLissage** est utilisée pour réaliser le lissage spatial pour l'application de cartographie 3D.

Pour chaque version (annee-ville-taille des carreaux) : 

* On créé une variable de contribution du carreau à la ségrégation totale
* On lisse la ségrégation et la contribution des carreaux (en lissant préalablement les populations des différents groupes sociaux et en recalculant proprement les indicateurs sur les populations lissées) 
* On discrétise les carreaux par niveaux de revenus et on associe une couleur pour la carto 

A noter que les données issues de cette opération sont restructurées et seules quelques variables sont sélectionnées pour être utilisées dans l'application de cartographie 3D. En effet, cette application étant héberger sur le Cluster Kube, les données carroyées lissées utilisées en input doivent être uploadées sur le cloud Minio interne. De ce fait, on limite au maximum les informations contenues dans les bases en input de l'application. 


### Création des données en input du tableau de bord

Tout comme l'application de cartographie 3D, le tableau de bord est une application Shiny hébergée sur le Cluster Kube interne. Les données en input sont hébergées sur le Cloud Minio interne. Comme Minio n'est pas aussi sécurisé que les coffres AUS, on ne dépose sur ce cloud que des données non-sensibles, sans aucun risque de révélation de secret.

Pour ce faire, on ne dépose sur Minio que les données agrégées destinées à être représentées dans le dashboard. La production de ces données agrégées est réalisée dans le script **05_data-dashboard.R**.



### Fonctionnement et hébergement des applications

Les applications de tableau de bord et de cartographie sont réalisées en RShiny.
Leur code source est versionné ici : 

* Cartographie : https://gitlab.insee.fr/psar-au/groupe_au33/au33carto
* Tableau de bord : https://gitlab.insee.fr/psar-au/groupe_au33/au-33-dashboard

Les données input de l'application de cartographie sont stockées :

* Sous AUS : X:\HAB-PSAR-AU-AU33-DEV\au33_v2\data\output\carto3d
* Puis uploadées sous Minio : https://onyxia.kube.developpement.insee.fr/mes-fichiers/f2smtb/data_au33carto3d/

Les données input de l'application de tableau de bord sont stockées :

* Sous AUS : X:\HAB-PSAR-AU-AU33-DEV\au33_v2\data\output\data_dashboard
* Puis uploadées sous Minio : https://onyxia.kube.developpement.insee.fr/mes-fichiers/f2smtb/data_au33dashboard/
