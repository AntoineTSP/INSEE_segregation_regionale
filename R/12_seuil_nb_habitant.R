...
#lmen doit être fourni et filtré par bdtopo
...
com_sous_seuil <- com_region %>% filter(Pop_tot_2019 < seuil_nb_hab)
com_au_dessus_seuil <- com_region %>% filter(Pop_tot_2019 >= seuil_nb_hab)

#On garde la liste des CODGEO des communes urbaines
Liste_CODGEO_sous_seuil <- com_sous_seuil$CODGEO
Liste_com_sous_seuil <- com_sous_seuil$LIBGEO
Nb_com_sous_seuil <- com_sous_seuil %>% count()

Liste_CODGEO_au_dessus_seuil <- com_au_dessus_seuil$CODGEO
Liste_com_au_dessus_seuil <- com_au_dessus_seuil$LIBGEO
Nb_com_au_dessus_seuil <- com_au_dessus_seuil %>% count()
#On filtre les contours
com_sous_seuil_contours <- com_contours %>%filter(codgeo %in% Liste_CODGEO_sous_seuil)
com_au_dessus_contours <- com_contours %>%filter(codgeo %in% Liste_CODGEO_au_dessus_seuil)
#### *********************************************
#### end ####

sf_au_dessus_region_inters <- sf_region %>% st_intersection(com_au_dessus_contours %>% select(codgeo,geometry))
...
territoire_au_dessus <- st_union(sf_au_dessus_region_inters$geometry)

# st_write(territoire_au_dessus,
#          paste0("U:/AU33/coupe_seuil/territoire/territoire_au_dessus_", as.character(seuil_nb_hab),  ".shp"))

sf_sous_seuil_region_inters <- sf_region %>% st_intersection(com_sous_seuil_contours %>% select(codgeo,geometry))

# # #Récupérer les contours des communes rurales  pour les voir sur QGIS:
# # st_write(sf_rural_region_inters, "U:/AU33/rural/territoire_rural.shp")
# territoire_sous_seuil <- st_union(sf_sous_seuil_region_inters$geometry)
territoire_sous_seuil <- st_union(sf_sous_seuil_region_inters$geometry)

# st_write(territoire_sous_seuil,
#          paste0("U:/AU33/coupe_seuil/territoire/territoire_sous_seuil_", as.character(seuil_nb_hab),  ".shp"))
...
# #R�cup�rer le revenu m�dian des m�nages ruraux / urbains
deb <- Sys.time()
list_df <- lmen %>% lapply(select_rows_mediane,territoire_sous_seuil)
liste_mediane_rurale <- lapply(list_df,function(df) median(df[[1]]$revdecucm))
liste_mediane_urbain <- lapply(list_df,function(df) median(df[[2]]$revdecucm))
fin <- Sys.time()-deb
fin #25min
...
clusterExport(cl, "sf_sous_seuil_region_inters")
...
lmen_region <- parLapply(cl,lmen_region,function(tab){
  l_tcar_named %>%
    lapply(f_carroyage,tab,nb_tranches,passageComZE = com_region,ZE_ssQP=pole_sans_rural,sf_sous_seuil_region_inters,"CODGEO", varCodeZe="REG")
})
...
# Sauvegarde des centroïdes de pôles pour les cartes
#Ajout region
...
# *************************************************************
#    Liste nommée des pôles pour les input des appli Shiny  ----
# *************************************************************
